...
def _build_prompt(req_data: dict, rag_chunks: list[str], config: dict) -> tuple[str, str]:
    ...
    for area in areas:
        ...
        temas_str = ", ".join(
            f"T{t['trimestre_num']}: {t['titulo']}" for t in temas_area
...
def generate_with_ollama(req_data: dict, rag_chunks: list[str], config: dict) -> dict:
    ...
    Lanza una excepción explícita si Ollama no responde — sin mock fallback.
    ...
    with httpx.Client(timeout=240.0) as client:
        ...
        if "application/json" not in content_type:
            raise ValueError(
                f"Ollama devolvió contenido no-JSON (content-type: {content_type}). "
                f"Verifica que el proxy permita POST a /api/generate. "
                f"Respuesta: {response.text[:300]}"
            )
        raw_response = response.json().get("response", "{}")
        contenido = json.loads(raw_response)
        if "areas" not in contenido:
            raise ValueError(f"Respuesta del LLM inválida — falta clave 'areas'. Recibido: {str(contenido)[:200]}")
