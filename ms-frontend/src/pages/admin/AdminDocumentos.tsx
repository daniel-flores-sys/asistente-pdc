...
import {
  adminDeleteDocumento,
  adminGetDocumentos,
  adminReindexDocumento,
  adminUploadDocumento,
} from '@/api';
...
export default function AdminDocumentos() {
  ...
  const upload = async (file: File) => {
    ...
    try {
      const res = await adminUploadDocumento(file);
      setUploadProgress(100);
      setSuccess(res.mensaje ?? `"${file.name}" subido correctamente.`);
      load();
    } catch (e: any) {
      setError(e.response?.data?.detail ?? e.response?.data?.message ?? 'Error al subir el archivo');
    } finally {
      clearInterval(timer);
      setUploading(false);
  ...
  const handleDelete = async (id: number, nombre: string) => {
    ...
    try {
      ...
      setError(e.response?.data?.detail ?? e.response?.data?.message ?? 'Error al eliminar');
    }
  };
  ...
  const handleReindex = async (id: number) => {
    ...
    try {
      ...
      setSuccess(res.mensaje ?? 'Reindexacion manual requerida.');
    } catch (e: any) {
      setError(e.response?.data?.detail ?? e.response?.data?.message ?? 'Error al reindexar');
    } finally {
    ...
    <div className="space-y-6">
      <div>
        ...
        <p className="text-muted-foreground text-sm">
          Sube PDF, DOCX o TXT para dar contexto al sistema de generacion.
        </p>
      </div>
      ...
      >
        <CardContent className="py-10 flex flex-col items-center gap-3 text-center">
          ...
          <div>
            <p className="font-medium text-sm">Arrastra un archivo aqui o haz clic para seleccionar</p>
      ...
      <Card>
        ...
        <CardContent className="p-0">
          {loading ? (
            ...
            <div className="text-center py-10 text-muted-foreground text-sm">No hay documentos aun.</div>
          ) : (
            <ul>
              {docs.map((doc, i) => (
                <li
                  key={doc.id}
                  className={`flex items-center gap-3 px-4 py-3 ${i !== docs.length - 1 ? 'border-b' : ''}`}
                >
                  <FileText className="w-4 h-4 text-muted-foreground shrink-0" />
                  <div className="flex-1 min-w-0">
                    <p className="text-sm font-medium truncate">{doc.nombre_archivo}</p>
                    <p className="text-xs text-muted-foreground">
                      {doc.chunks_generados} chunks · {new Date(doc.creado_en).toLocaleDateString('es-BO')}
                    </p>
                  </div>
                  <Badge variant="outline" className="text-xs">
                    {ext(doc.nombre_archivo)}
                  </Badge>
                  <Badge variant={doc.estado === 'indexado' ? 'secondary' : 'outline'} className="text-xs">
                    {doc.estado}
                  </Badge>
                  <Button
                    variant="ghost"
