...
export default function GenerarWizard() {
  ...
  // Paso 2 — UUIDs como string
  const [anioId, setAnioId] = useState('');
  const [trimId, setTrimId] = useState('');

  // Paso 3 — UUIDs como string
  const [areasSelected, setAreasSelected] = useState<string[]>([]);
  const [temasSelected, setTemasSelected] = useState<Record<string, string[]>>({});
  ...
  const toggleTema = (areaId: string, temaId: string) =>
  ...
  const handleSubmit = async () => {
    ...
    try {
      const res = await generatePDC({
        ...
        temas_seleccionados: temasSelected,
        materiales,
        ...
        <Card className="max-w-md w-full text-center">
          <CardContent className="pt-8 pb-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground text-sm">Plan listo</p>
            </div>
          ...
          <>
            ...
            <CardContent className="space-y-4">
              ...
              <div className="space-y-1.5">
                ...
                <Select value={trimId} onValueChange={setTrimId}>
                  <SelectTrigger>
                  ...
                  <SelectContent>
                    {(refData.trimestres ?? []).map((t) => (
                      <SelectItem key={t.id} value={t.id}>
                        Trimestre {t.numero} — {t.gestion_anio} ({t.fecha_inicio?.slice(0,10)} al {t.fecha_fin?.slice(0,10)})
                      </SelectItem>
                    ))}
                  </SelectContent>
                </Select>
        ...
        {step === 3 && (
          <>
            <CardHeader>
              ...
              <p className="text-sm text-muted-foreground">Selecciona las áreas y los temas del trimestre.</p>
            </CardHeader>
                          ...
                          temasFiltrados.map((t) => (
                            <div key={t.id} className="flex items-start gap-2">
                              ...
                              <label htmlFor={`tema-${t.id}`} className="text-xs cursor-pointer text-muted-foreground leading-5">
                                <span className="font-medium text-foreground">T{t.trimestre_num}:</span> {t.titulo}
                              </label>
                            </div>
                          ))
                        ) : (
