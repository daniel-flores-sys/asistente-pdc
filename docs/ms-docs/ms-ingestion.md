...
| Embeddings | Ollama (nomic-embed-text) via Server 2; fallback hash-deterministico sin dependencias |
...
4. Embeddings (768 dimensiones):
   - Primario: POST http://{OLLAMA_URL}/api/embeddings con nomic-embed-text
   - Fallback (si Ollama no responde): hash SHA-512 deterministico sin dependencias externas
     → vectores normalizados, no semánticos pero funcionales para ChromaDB
