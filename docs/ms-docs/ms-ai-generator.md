...
| LLM | Ollama gemma4:e4b vía proxy https://ollama-server-246.rootcode.com.bo |
...
OLLAMA_URL=https://ollama-server-246.rootcode.com.bo
...
POST /generate
  Body: {
    ...
    anio_escolaridad_id: string,   // UUID
    trimestre_id:        string,   // UUID
    areas: [{
      id:     string,              // UUID
      nombre: string,
      codigo: string
...
```
1. INSERT plan_curricular con UUIDs (usuario_id, anio_escolaridad_id, trimestre_id ya resueltos por el orchestrator)
2. Consulta ChromaDB:
...
4. POST Ollama /api/generate → espera respuesta JSON del LLM (timeout 240s)
   - Si el proxy devuelve HTML (content-type != application/json) → ValueError con mensaje claro
   - Sin mock fallback: si Ollama no responde, el endpoint lanza error 500
...
```python
# Claves que lee:
'llm_params'  → { temperatura: 0.7, max_tokens: 4096, modelo: "gemma4:e4b" }
