...
GET /api/reference-data
  Response: {
    ...
    temas_trimestral: [{ id, area_id, anio_id, trimestre_num, titulo, descripcion }]
  }
```
...
POST /api/generate
  Body: {
    ...
    areas_seleccionadas: string[], // UUID[]
    temas_seleccionados: { [area_uuid: string]: string[] },  // UUID[]
    materiales?:         string,
...
PUT  /api/admin/usuarios/:id/creditos    # :id = UUID
  Body: { creditos: number }
PUT  /api/admin/usuarios/:id/activo      # :id = UUID
  Body: { activo: boolean }
...
DELETE /api/admin/documentos/:id          # :id = UUID
...
3. Resolución desde PG: objetivo_holistico, nombres de áreas (por UUID[]), temas (por UUID[])
   → construye payload con datos resueltos antes de llamar al AI generator
4. `AiGeneratorService.generate(payload)` → POST ms-ai-generator `/generate` (timeout 240s)
