...
export class DocumentosAdminController {
  ...
  async list(@Res() res: Response) {
    try {
      ...
      console.error('[DocumentosAdminController] Error en list:', error);
  ...
  async upload(
    ...
    try {
      ...
      // Pasa el ID del admin autenticado para que ms-ingestion lo registre en subido_por
      // El JWT payload usa 'sub' como campo del ID (no 'id')
      const adminId = (req as any).user?.sub;
      const ingestUrl = adminId
        ? `${this.ingestionUrl}/ingest?admin_id=${adminId}`
        : `${this.ingestionUrl}/ingest`;

      const upstream = await axios.post(ingestUrl, form, {
  ...
  async reindex(@Param('id', ParseIntPipe) id: number, @Res() res: Response) {
    try {
      ...
      console.error('[DocumentosAdminController] Error en reindex:', error);
      const msg = error instanceof Error ? error.message : String(error);
      throw new HttpException({ error: 'ms-ingestion no disponible', detalle: msg }, HttpStatus.BAD_GATEWAY);
    }
  }
